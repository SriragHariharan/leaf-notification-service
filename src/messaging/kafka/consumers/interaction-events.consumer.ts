...
import type { InteractionEvent } from "../contracts/interaction-event.dto";
...
const consumer = kafka.consumer({ groupId: "notification-service-interaction-events" });

async function onMessage(payload: EachMessagePayload): Promise<void> {
  const raw = payload.message.value?.toString() ?? "";
  const event = parseJsonMessage<InteractionEvent>(raw, TOPIC);
  if (!event?.eventType) {
...
async function startInteractionEventsConsumer(): Promise<void> {
  try {
...
startInteractionEventsConsumer();
