...
async function createUser(user: any): Promise<boolean> {
    try {
        ...
        const newUser = new User({ userID: user?.userID, username: user?.username, profilePic: user?.profilePicture });
        await newUser.save();
...
async function updateProfilePicture(userID: string, newProfilePicture: string): Promise<boolean> {
    try {
        ...
        await User.updateOne({ userID }, { $set: { profilePic: newProfilePicture } });
